...
- última reconciliación aceptada del Malāk Project Vault con Malāk
  `main@5865da6a5e502fe71e35e2e38bc4cceaab9b3600` mediante PR #98; los cambios
  posteriores hasta el HEAD actual permanecen pendientes de reconciliación
  downstream.
...
Última unidad de producto:       G2A — Protected Finalization Foundation
...
Implementación posterior autorizada: ninguna posterior a G2A
...
PR #98/#99 integraron G0/G1 documental de Progressive Cognitive Assurance. La
promoción normativa posterior quedó registrada mediante ADR-005, Blueprint
v0.6.2-alpha y Cognitive Constitution v1.1.0.

PR #110 integró `G2A — Protected Finalization Foundation` como foundation de
código aislada, determinista y sin wiring a Conversation, Kernel, provider o
runtime. PR #111 integró G0/G1 de `Assurance Signal Authority Boundary`; Signal
Boundary G2 y Conversation Finalization G2B permanecen no autorizados.
...
cambios posteriores hasta este baseline documental deben reconciliarse antes del
siguiente gate que requiera drift cero. El estado del Vault no concede autoridad
...
PR #110 integró una foundation aislada bajo:

```text
src/malak/core/protected_finalization.py
```

La frontera representa explícitamente:
...
El evaluator es puro y determinista, no accede a provider/runtime, no modifica
Kernel ni Conversation y no produce por sí mismo las señales de assurance. PR
#111 documentó G0/G1 del ownership de esas señales. Signal Boundary G2 y
Conversation Finalization G2B permanecen fuera de alcance y no autorizados.
...
El current `main@e45a3e3c0ebf657a513596aa74452413479c05d1` tiene Validation
post-merge en `success`.
...
la Memory Layer de forma aislada hasta Governed Projection Consumption y G2A
materializó una foundation cognitiva aislada sin conectarla a Conversation.
...
Signal Boundary G2 o Conversation Finalization G2B sin un proceso específico de
...
y quedó integrada en Vault mediante PR #98, con Vault
`main@34f0416a8d312f4f27b9b36c3733cc2703772364`.

Los cambios integrados después de `5865da6a` hasta el baseline actual deben
...
Cognitive Assurance; su promoción normativa posterior quedó reflejada mediante
ADR-005, Blueprint v0.6.2-alpha y Cognitive Constitution v1.1.0.

PR #110 integró G2A — Protected Finalization Foundation en aislamiento. PR #111
integró G0/G1 de Assurance Signal Authority Boundary. Signal Boundary G2 y
Conversation Finalization G2B permanecen no autorizados.
...
- Signal Boundary G2 o Conversation Finalization G2B sin gate y autorización separados;
