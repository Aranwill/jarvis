...
| `EXT-25` | TypeSafe AI — *Introducing System One Models & Jev* — 2026-09-15 | estado no estructurado → decisiones tipadas probabilísticas, outputs paralelos y RLCD declarado por el proveedor | una frontera cognitiva puede restringir outputs a tipos conocidos y dejar branching, thresholds y autoridad en código/política determinista | Cognitive Minimization / Protected Finalization / Model Governance | `CORROBORATION` + `IMPLEMENTATION_INPUT` + `WATCH_SIGNAL` | adoptar Jev, asumir que confidence=correctness, aceptar "zero hallucinations" como garantía semántica o inferir arquitectura/RLCD no publicados |
| `EXT-26` | TrueStandard — evaluación independiente de Jev, 108 claims — 2026-09-18 | accuracy, Brier score y calibration error comparados contra dos modelos chat; resultados sensibles a muestra/prompt | typed output reduce errores de forma, pero probabilidad/confidence requiere calibración empírica sobre la distribución real antes de automatizar decisiones | Cognitive Assurance / Evidence-Bound Finalization / evaluation | `CORROBORATION` + `WATCH_SIGNAL` | convertir un benchmark pequeño en verdad general, umbral universal o autorización de acción |
